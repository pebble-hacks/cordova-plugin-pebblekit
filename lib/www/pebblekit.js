...
function registerDataLogReceiver(uuid, successCallback, errorCallback, keepAlive) {
  if (!uuid) {
    console.error('First argument must be uuid of watch app');
    return;
  }
  ...
  if (!errorCallback) errorCallback = genericErrorHandler;
  if (!keepAlive) keepAlive = false;
  exec(successCallback, errorCallback, CLASS_NAME, 'registerDataLogReceiver', [uuid, keepAlive]);
}

function unregisterDataLogReceiver(successCallback, errorCallback) {
  if (!errorCallback) errorCallback = genericErrorHandler;
  exec(successCallback, errorCallback, CLASS_NAME, 'unregisterDataLogReceiver', []);
}
